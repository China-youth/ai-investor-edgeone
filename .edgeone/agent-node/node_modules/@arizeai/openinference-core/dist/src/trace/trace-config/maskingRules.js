 ...
 */
function isWithinImagesNamespace(key, namespace) {
 ...
 *  }) // returns true so the rule applies and the value will be removed
 */
const maskSpanInputImagesRule = {
    condition: ({ config, key }) => (config.hideInputs || config.hideInputImages) &&
        isWithinImagesNamespace(key, openinference_semantic_conventions_1.SemanticConventions.INPUT_IMAGES),
    action: () => undefined,
};
/**
 ...
 *  }) // returns true so the rule applies and the value will be removed
 */
const maskSpanOutputImagesRule = {
    condition: ({ config, key }) => config.hideOutputs && isWithinImagesNamespace(key, openinference_semantic_conventions_1.SemanticConventions.OUTPUT_IMAGES),
    action: () => undefined,
};
...
const maskLongBase64ImageRule = {
    condition: ({ config, key, value }) => typeof value === "string" &&
        ...
        ((key.includes(openinference_semantic_conventions_1.SemanticConventions.LLM_INPUT_MESSAGES) &&
            key.includes(openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENT_IMAGE)) ||
            isWithinImagesNamespace(key, openinference_semantic_conventions_1.SemanticConventions.INPUT_IMAGES) ||
            isWithinImagesNamespace(key, openinference_semantic_conventions_1.SemanticConventions.OUTPUT_IMAGES)),
    action: () => constants_1.REDACTED_VALUE,
};
/**
