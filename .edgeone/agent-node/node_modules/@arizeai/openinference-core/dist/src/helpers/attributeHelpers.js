...
exports.getAnnotationAttributes = getAnnotationAttributes;
exports.getEvaluationAttributes = getEvaluationAttributes;
...
function getDocumentAttributes(document, documentIndex, keyPrefix) {
    ...
    const attributes = {};
    if (document.content != null) {
        attributes[`${keyPrefix}.${documentIndex}.${openinference_semantic_conventions_1.SemanticConventions.DOCUMENT_CONTENT}`] =
            document.content;
    }
    if (document.id != null) {
    ...
    if (document.score != null) {
        attributes[`${keyPrefix}.${documentIndex}.${openinference_semantic_conventions_1.SemanticConventions.DOCUMENT_SCORE}`] =
            document.score;
    }
    return attributes;
}
const annotationCollectionPrefixes = {
    annotation: {
        span: openinference_semantic_conventions_1.SemanticConventions.ANNOTATIONS,
        trace: openinference_semantic_conventions_1.SemanticConventions.TRACE_ANNOTATIONS,
        session: openinference_semantic_conventions_1.SemanticConventions.SESSION_ANNOTATIONS,
    },
    evaluation: {
        span: openinference_semantic_conventions_1.SemanticConventions.EVALUATIONS,
        trace: openinference_semantic_conventions_1.SemanticConventions.TRACE_EVALUATIONS,
        session: openinference_semantic_conventions_1.SemanticConventions.SESSION_EVALUATIONS,
    },
};
const annotationFieldNames = {
    annotation: {
        name: openinference_semantic_conventions_1.SemanticConventions.ANNOTATION_NAME,
        score: openinference_semantic_conventions_1.SemanticConventions.ANNOTATION_SCORE,
        label: openinference_semantic_conventions_1.SemanticConventions.ANNOTATION_LABEL,
        explanation: openinference_semantic_conventions_1.SemanticConventions.ANNOTATION_EXPLANATION,
        annotatorKind: openinference_semantic_conventions_1.SemanticConventions.ANNOTATION_ANNOTATOR_KIND,
        identifier: openinference_semantic_conventions_1.SemanticConventions.ANNOTATION_IDENTIFIER,
        metadata: openinference_semantic_conventions_1.SemanticConventions.ANNOTATION_METADATA,
    },
    evaluation: {
        name: openinference_semantic_conventions_1.SemanticConventions.EVALUATION_NAME,
        score: openinference_semantic_conventions_1.SemanticConventions.EVALUATION_SCORE,
        label: openinference_semantic_conventions_1.SemanticConventions.EVALUATION_LABEL,
        explanation: openinference_semantic_conventions_1.SemanticConventions.EVALUATION_EXPLANATION,
        annotatorKind: openinference_semantic_conventions_1.SemanticConventions.EVALUATION_ANNOTATOR_KIND,
        identifier: openinference_semantic_conventions_1.SemanticConventions.EVALUATION_IDENTIFIER,
        metadata: openinference_semantic_conventions_1.SemanticConventions.EVALUATION_METADATA,
    },
};
...
function getScopedAnnotationAttributes(options) {
    ...
    const collectionPrefix = (_a = annotationCollectionPrefixes[terminology]) === null || _a === void 0 ? void 0 : _a[scope];
...
function getAnnotationAttributes(options) {
    ...
    return getScopedAnnotationAttributes({
        ...
        scope: (_a = options.scope) !== null && _a !== void 0 ? _a : "span",
    });
}
/**
...
function getToolAttributes(options) {
    ...
    attributes[openinference_semantic_conventions_1.SemanticConventions.TOOL_PARAMETERS] = (_a = (0, utils_1.safelyJSONStringify)(parameters)) !== null && _a !== void 0 ? _a : "{}";
    return attributes;
}
/**
...
function getModelNameAttributes(options) {
    var _a, _b;
    const attributes = {};
    const modelName = (_b = (_a = options.modelName) !== null && _a !== void 0 ? _a : options.responseModelName) !== null && _b !== void 0 ? _b : options.requestModelName;
    if (modelName != null) {
        attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_MODEL_NAME] = modelName;
    }
    if (options.requestModelName != null) {
        attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_REQUEST_MODEL_NAME] = options.requestModelName;
    }
    if (options.responseModelName != null) {
        attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_RESPONSE_MODEL_NAME] = options.responseModelName;
    }
    return attributes;
}
/**
...
function getLLMAttributes(options) {
    ...
    if (options.provider != null) {
        attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_PROVIDER] = options.provider.toLowerCase();
    }
    // System attributes
    ...
    if (options.invocationParameters != null) {
        attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_INVOCATION_PARAMETERS] = (_a = (0, utils_1.safelyJSONStringify)(options.invocationParameters)) !== null && _a !== void 0 ? _a : "{}";
    }
    // Input messages
                ...
                message.contents.forEach((content, contentIndex) => {
                    ...
                    if (content.type === "reasoning") {
                        ...
                        if (content.signature != null) {
                            attributes[`${openinference_semantic_conventions_1.SemanticConventions.LLM_INPUT_MESSAGES}.${messageIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENTS}.${contentIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENT_SIGNATURE}`] = content.signature;
                        }
                        if (content.data != null) {
                            attributes[`${openinference_semantic_conventions_1.SemanticConventions.LLM_INPUT_MESSAGES}.${messageIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENTS}.${contentIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENT_DATA}`] = content.data;
                        }
            ...
            if (Array.isArray(message.toolCalls)) {
                message.toolCalls.forEach((toolCall, toolCallIndex) => {
                    ...
                    if (toolCall.reasoningSignature != null) {
                        attributes[`${openinference_semantic_conventions_1.SemanticConventions.LLM_INPUT_MESSAGES}.${messageIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_TOOL_CALLS}.${toolCallIndex}.${openinference_semantic_conventions_1.SemanticConventions.TOOL_CALL_REASONING_SIGNATURE}`] = toolCall.reasoningSignature;
                    }
                });
            }
        });
            ...
            if (Array.isArray(message.contents)) {
                message.contents.forEach((content, contentIndex) => {
                    ...
                    if (content.type === "reasoning") {
                        ...
                        if (content.signature != null) {
                            attributes[`${openinference_semantic_conventions_1.SemanticConventions.LLM_OUTPUT_MESSAGES}.${messageIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENTS}.${contentIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENT_SIGNATURE}`] = content.signature;
                        }
                        if (content.data != null) {
                            attributes[`${openinference_semantic_conventions_1.SemanticConventions.LLM_OUTPUT_MESSAGES}.${messageIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENTS}.${contentIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENT_DATA}`] = content.data;
                        }
                        if (content.encryptedContent != null) {
                            attributes[`${openinference_semantic_conventions_1.SemanticConventions.LLM_OUTPUT_MESSAGES}.${messageIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENTS}.${contentIndex}.${openinference_semantic_conventions_1.SemanticConventions.MESSAGE_CONTENT_ENCRYPTED_CONTENT}`] = content.encryptedContent;
                        }
    ...
    if (options.tokenCount != null) {
        if (options.tokenCount.prompt != null) {
            attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_TOKEN_COUNT_PROMPT] = options.tokenCount.prompt;
        }
        if (options.tokenCount.completion != null) {
            attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_TOKEN_COUNT_COMPLETION] = options.tokenCount.completion;
        }
        if (options.tokenCount.total != null) {
            attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_TOKEN_COUNT_TOTAL] = options.tokenCount.total;
        }
        if (options.tokenCount.promptDetails != null) {
            const details = options.tokenCount.promptDetails;
            if (details.audio != null) {
                attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_TOKEN_COUNT_PROMPT_DETAILS_AUDIO] = details.audio;
            }
            if (details.cacheRead != null) {
                attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_TOKEN_COUNT_PROMPT_DETAILS_CACHE_READ] =
                    details.cacheRead;
            }
            if (details.cacheWrite != null) {
                attributes[openinference_semantic_conventions_1.SemanticConventions.LLM_TOKEN_COUNT_PROMPT_DETAILS_CACHE_WRITE] =
                    details.cacheWrite;
            }
        }
    }
