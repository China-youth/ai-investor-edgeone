...
import { GRAPH_NODE_ID, GRAPH_NODE_PARENT_ID, INPUT_MIME_TYPE, INPUT_VALUE, LLM_INPUT_MESSAGES, LLM_INVOCATION_PARAMETERS, LLM_MODEL_NAME, LLM_OUTPUT_MESSAGES, LLM_SYSTEM, LLM_TOKEN_COUNT_COMPLETION, LLM_TOKEN_COUNT_COMPLETION_DETAILS_REASONING, LLM_TOKEN_COUNT_PROMPT, LLM_TOKEN_COUNT_PROMPT_DETAILS_CACHE_READ, LLM_TOKEN_COUNT_TOTAL, LLM_TOOLS, LLMSystem, MESSAGE_CONTENT, MESSAGE_CONTENT_TEXT, MESSAGE_CONTENT_TYPE, MESSAGE_CONTENTS, MESSAGE_ROLE, MESSAGE_TOOL_CALL_ID, MESSAGE_TOOL_CALLS, MimeType, OpenInferenceSpanKind, OUTPUT_MIME_TYPE, OUTPUT_VALUE, SemanticConventions, TOOL_CALL_FUNCTION_ARGUMENTS_JSON, TOOL_CALL_FUNCTION_NAME, TOOL_CALL_ID, TOOL_JSON_SCHEMA, TOOL_NAME, } from "@arizeai/openinference-semantic-conventions";
...
function extractFromChatCompletionResponses(responses) {
    ...
    if (hasUsage) {
        if (totalPrompt > 0)
            attrs[LLM_TOKEN_COUNT_PROMPT] = totalPrompt;
        if (totalCompletion > 0)
            attrs[LLM_TOKEN_COUNT_COMPLETION] = totalCompletion;
...
function getResponseAttributes(data) {
    ...
    if (isRecord(resp.usage)) {
        ...
        if (isNumber(usage.total_tokens))
            attrs[LLM_TOKEN_COUNT_TOTAL] = usage.total_tokens;
